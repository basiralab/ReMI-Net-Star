...
def get_real_data(hemisphere="LH",labels=False,anti_vec=True):
    # Implement the real data function that return 2 numpy arrays:
    # Samples shaped: (n_samples, n_timepoints, n_nodes(ROI), n_nodes(ROI), n_edge_features(views))
    # Labels array for each sample. (n_samples)
    # return samples, labels
    pass

if __name__ == "__main__":
    X = prepare_data(new_data=True,n_samples=50,n_times=3)