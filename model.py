...
from graph_recurrent_conv import DoubleRNNConv, EdgeRNNConv
...
class RemiNet(Module):
    def __init__(self, n_nodes, input_size, hidden_sizes, recursion="cyclic",norm_method = "sigmoidnorm", convolution = "double_rnn"):
        super(RemiNet, self).__init__()

        # Ensure parameters are properly defined.
        assert recursion == "cyclic" or recursion == "vanilla"
        assert convolution == "double_rnn" or convolution == "edge_rnn"
        assert norm_method == "sigmoidnorm" or norm_method == "minmax" or norm_method == "no_norm"

        self.recursion = recursion
        self.norm_method = norm_method

        if self.norm_method == "minmax": self.minmax_norm_layer = MinMaxLayerNorm([n_nodes*n_nodes,input_size]) # (1225, 4)
        if self.norm_method == "sigmoidnorm": self.signorm_layer = Sequential(LayerNorm([n_nodes*n_nodes,input_size]), Sigmoid()) # (1225, 4)

        # Define 3 ReMI-Net Layers.
        if convolution == "double_rnn": 
        ...
        if convolution == "edge_rnn": 
            rnn = RNNCell(input_size, hidden_sizes[0]*2) # (4, 12*2)
            self.rec_conv1 = EdgeRNNConv(hidden_sizes[0], rnn)

            rnn = RNNCell(input_size, hidden_sizes[1]*2) # (4, 36*2)
            self.rec_conv2 = EdgeRNNConv(hidden_sizes[1], rnn)

            rnn = RNNCell(input_size, hidden_sizes[2]*2) # (4, 24*2)
            self.rec_conv3 = EdgeRNNConv(hidden_sizes[2], rnn)
