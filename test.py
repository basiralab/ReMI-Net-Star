...
def test_reminet(X, n_folds=5,timepoints=2,data_name="simulated", model_type="cyclic", norm="minmax",conv="edge_rnn"):
    test_losses = []
    reg_losses = []
    ...
    for i in range(n_folds):
        ...
        # Load trained model.
        model = RemiNet(shape[-2],shape[-1],[h1_size,h2_size,h3_size], model_type, norm, conv) # All parameters must be exactly the same with the model that you are trying to load.
        model.load_state_dict(torch.load(f"./models/{data_name}data_{model_type}_{norm}_{conv}_model_fold{i}"))
        model.to(device)
...
if __name__=="__main__":
    ...
    test_losses, reg_losses = test_reminet(X,timepoints=X.shape[1],data_name="simulated",norm="minmax",conv="edge_rnn")