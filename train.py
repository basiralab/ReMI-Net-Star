...
from dataset import prepare_data#, get_real_data
...
def train(X, n_max_epochs, time_points = 3, sampling_size = 10, n_folds = 5, dataset="simulated", model_type="cyclic",norm_method="sigmoidnorm",convolution="double_rnn"):
    ...
    for i in range(n_folds):
        ...
        model = RemiNet(shape[-2], shape[-1], [h1_size,h2_size,h3_size], model_type, norm_method, convolution).to(device)
        ...
        for epoch in range(n_max_epochs):
            ...
            torch.cuda.synchronize() # You can comment this line out if you work on cpu.
            optimizer.step()
        ...
        torch.save(model.state_dict(), f"./models/{dataset}data_{model_type}_{norm_method}_{convolution}_model_fold{i}")
    ...
    # Save all results.
    np.save(f"./experiments/{dataset}data_L1_{model_type}_{norm_method}_{convolution}_model.npy", Lc_losses)
    np.save(f"./experiments/{dataset}data_L2_{model_type}_{norm_method}_{convolution}_model.npy", Lt_losses)
    return Lc_losses, Lt_losses

            
if __name__=="__main__":
    # dataset can be anything that you want to use for the name of the model.
    # model_type must be "cyclic" or "vanilla"
    # norm_method must be "no_norm", "sigmoidnorm" or "minmax"
    # conv must be "double_rnn" or "edge_rnn"
    X = prepare_data()
    Lc_losses, Lt_losses = train(X,100,X.shape[1],dataset="simulated",model_type="cyclic",norm_method="sigmoidnorm", convolution="edge_rnn")
